---
title: Open methodology for "Using column generation in constraint-and-column generation for adjustable robust optimization" > FLP
output: 
  html_document:
    theme: null
...
```{r}
data = read.csv("results_new.csv", header = FALSE)
...
```{r}
ggplot(data, aes(x = total_time, col = method)) + stat_ecdf(pad = FALSE) + coord_cartesian(xlim = c(0,10800))
```
...
```{r}
# Calculate the summary statistics for instances with total_time < 10800
...
```{r}
data[data$total_time < 10800 & data$adversarial_unexpected_status != "" & data$with_heuristic == TRUE,]
```
