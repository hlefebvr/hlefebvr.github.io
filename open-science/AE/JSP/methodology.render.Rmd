---
title: Open methodology for "Using column generation in constraint-and-column generation for adjustable robust optimization" > JSP
output: 
  html_document:
    theme: null
...
ccg = data[data$method == "STD_TLInf_H0" & data$total_time < 7200,]

ggplot(ccg, aes(x = as.factor(Gamma), y = n_iterations)) +
  geom_boxplot() +
  facet_grid(. ~ n_jobs) +
  labs(x = "|J|", y = "Number of scenarios") +
  theme_minimal()

ggplot(ccg, aes(x = master_time, color = factor(Gamma))) +
  stat_ecdf(geom = "step") +
  xlab("n_iterations") +
  ylab("ECDF") +
  labs(color = "Gamma") +
  scale_color_manual(values = c("red", "blue", "green")) # You can specify your own colors
...
result <- data[data$total_time < 7200,] %>%
  group_by(instance, Gamma) %>%
  summarize(same_best_obj = sd(best_obj))

result
```

```{r}
data[data$total_time < 7200 & data$adversarial_unexpected_status != "" & data$with_heuristic == TRUE,]
```
