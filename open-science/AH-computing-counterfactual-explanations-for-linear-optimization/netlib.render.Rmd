...
for (norm_val in unique(data$norm)) {
  ...
  plot = ggplot(sub_data, aes(x = l1_norm, y = as.factor(solver), fill = as.factor(solver))) + 
    ...
    theme_minimal() +
    theme(legend.position = "none", axis.text.y = element_text(angle = 0, hjust = .5)) # +
...
```{r}
ws_gap = data %>% filter(method == "PADM", !is.na(warm_start_l1_norm), !is.na(l1_norm)) %>%
    mutate(gap_warm_start = (warm_start_l1_norm - l1_norm) / (1e-10 + warm_start_l1_norm)) %>%
    select(full_instance, warm_start_l1_norm, l1_norm, gap_warm_start)
