...
p = plot_ecdf_of_time(all_results %>% filter(!startsWith(method, "PADM-interdiction")), log = TRUE)
p
...
plot_ecdf_of_gap_wrt_best_known(solved_by_both, t_status = "open with feasible point")

for (t_method in c("PADM-100", "MILP-first", "MibS-First")) {
  solved_by_both = solved_by_both = all_results %>% filter(method == t_method) %>%
...
slow_instances <- all_results %>%
  filter(method == "MibS-First", time > 120) %>%
  distinct(instance)
...
df <- filtered %>%
  ...
  mutate(rel_gap = ifelse(time > TIME_LIMIT_HEUR | best_obj > 1e19 | is.na(best_obj) | is.na(best_obj_instance), MAX_REL_GAP, 100 * abs(best_obj - best_obj_instance) / (1e-10 + abs(best_obj_instance)))) %>%
...
ggplot(df, aes(x = rel_gap, color = method)) +
  stat_ecdf(size = 1)  +
...
all_results_with_status = all_results %>%
  filter(method == "MILP-first" & time < 3600) %>%
  mutate(
