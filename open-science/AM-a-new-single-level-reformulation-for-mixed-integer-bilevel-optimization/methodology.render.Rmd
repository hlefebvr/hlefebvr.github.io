...
get_results_from_hpc = function(method) {
  ...
  if (method == "PADM") {
    ...
    colnames(result) = c("tag", "instance", "method", "time", "status", "reason", "best_obj", "best_bound", "lambda", "pi_lb", "pi_ub") 
  }
  
  result = result %>% mutate(instance = basename(instance), instance = gsub("\\.aux$", "", instance))
  result = result %>% mutate(time = ifelse(time < -1e-3 | time > 3600, 3600, time))
...
mibs_first = read.csv("first_ub_mibs.csv", header = TRUE, sep = ",") %>% select(all_of(columns))
mibs_first = mibs_first %>% mutate(time = ifelse(time > 3600, 3600, time))
mibs_first = mibs_first %>% mutate(best_obj = ifelse(time > 3600, 1e20, best_obj))
...
mibs_first = add_missing_results(mibs_first)

# Aggregating all results
all_results = rbind(
  mibs, milp, milp_first, padm, padm_interdiction, mibs_first
)
...
```{r}
#all_results = all_results %>% filter(class != "interdiciton")
...
all_results_with_size <- all_results %>%
  filter(method %in% c("MibS", "MILP", "MibS-First", "MILP-first", "PADM-1000")) %>%
...
plot_ecdf_of_time = function(data, log = FALSE) {
  p = ggplot(data, aes(x = time + 1, color = method)) +
...
p = plot_ecdf_of_time(all_results %>% filter(!startsWith(method, "PADM-interdiction")), log = FALSE) # , class == "interdiction"
p
...
plot_ecdf_of_gap_wrt_best_known = function(data, t_type = NULL, t_class = NULL, t_status = "optimal") {
  results_with_gaps = data %>%
  filter(!is.na(best_known_obj)) %>% 
  filter(bobilib_status == t_status) %>%
  filter(time < 3500) %>%
  mutate(gap = (best_obj - best_known_obj) / (1e-10  + abs(best_known_obj))) #%>%
  #mutate(gap = ifelse(gap < 1e-4, 1e-4, gap))
  
  if (!is.null(t_type)) {
    results_with_gaps = results_with_gaps %>% filter(type == t_type)
  }
  
  if (!is.null(t_class)) {
    results_with_gaps = results_with_gaps %>% filter(class == t_class)
  }
  ...
  p = ggplot(results_with_gaps, aes(x = gap, color = method)) +
    stat_ecdf(geom = "step", linewidth = 1) +
    #scale_x_log10() +
    coord_cartesian(xlim = c(-1, 1e1)) +
    labs(
      title = paste("ECDF of Gap / ", t_type, "/", t_class),
      x = "Gap (%)",
      y = "Instances (%)"
    ) +
    theme_minimal() +
...
solved_by_both = solved_by_both = all_results %>% filter(method == "PADM-100" | method == "PADM-1000" | method == "MILP-first" | method == "MibS-First") %>%
  group_by(instance) %>%
...
print(p)
export_data_from_ggplot_ecdf_csv(p, "ecdf_gap_bobilib.csv")

for (class in unique(all_results$class)) {
  p = plot_ecdf_of_gap_wrt_best_known(solved_by_both, t_class = class)
  print(p)
}

for (t_class in unique(all_results$class)) {
  solved_by_both = solved_by_both = all_results %>% filter(class == t_class) %>%
    filter(time < 3600 & time >= 0) %>%
    ungroup()
...
```{r}
MAX_REL_GAP = 1e2
TIME_LIMIT_HEUR = 3600
MIN_TIME = 120

slow_instances <- all_results %>%
  filter(method == "MibS-First", time >= MIN_TIME) %>%
  distinct(instance)

filtered = all_results %>% filter(instance %in% slow_instances$instance)

best_per_instance <- filtered %>%
  filter(method != "PADM-interdiction-100" & method != "PADM-interdiction-1000") %>%
  group_by(instance) %>%
  summarise(best_obj_instance = min(best_obj, na.rm = TRUE))

df <- filtered %>%
  filter(method != "PADM-interdiction-100" & method != "PADM-interdiction-1000" & method != "MibS" & method != "MILP") %>%
  left_join(best_per_instance, by = "instance") %>%
  mutate(rel_gap = ifelse(best_obj > 1e19 | is.na(best_obj) | is.na(best_obj_instance), MAX_REL_GAP, 100 * abs(best_obj - best_obj_instance) / (1e-10 + abs(best_obj_instance)))) %>%
  mutate(rel_gap = ifelse(abs(rel_gap) < 1e-4, 1e-4, rel_gap)) %>%
  mutate(rel_gap = ifelse(rel_gap > MAX_REL_GAP, MAX_REL_GAP, rel_gap))

length(unique(df$instance))

r_max <- max(df$rel_gap[is.finite(df$rel_gap)], na.rm = TRUE)

p = ggplot(df, aes(x = rel_gap, color = method)) +#  
  stat_ecdf(size = 1)  +
  #coord_cartesian(xlim = c(0, 12.5)) +
  #scale_x_log10() +
  labs(
    title = "Performance Profile (Relative Gap (%))",
    x = "Relative performance ratio",
    y = "ECDF",
    color = "Method"
  ) +
  theme_minimal()
print(p)
export_data_from_ggplot_ecdf_csv(p, "ecdf_gap_all_after_120.csv")
```
