...
plot_ecdf_of_time(all_results %>% filter(method != "PADM-interdiction"))
```
...
plot_ecdf_of_gap_wrt_best_known(all_results %>% filter(method == "PADM"))


solved_by_both = solved_by_both = all_results %>% filter(method == "PADM" | method == "MILP-first") %>%
  group_by(instance) %>%
  filter(all(time < 3600 & time >= 0)) %>%
  ungroup()

plot_ecdf_of_gap_wrt_best_known(solved_by_both)
```
...
all_results_with_status = all_results %>%
  ...
  mutate(
    ...
    new_bobilib_status = ifelse(bobilib_status == "optimal", "was optimal\n and found feasible point", new_bobilib_status),
    new_bobilib_status = ifelse(new_bobilib_status == "open with feasible point", "was open with feasible point\n and found non-improving\n feasible point", new_bobilib_status)
  )
...
solved_by_both = all_results %>% filter(method == "PADM" | method == "PADM-interdiction") %>%
  group_by(instance) %>%
  filter(all(time >= 0)) %>%
  ungroup()

solved_by_both = solved_by_both = all_results %>% filter(method == "PADM" | method == "PADM-interdiction") %>%
  group_by(instance) %>%
  filter(all(time < 3600 & time >= 0)) %>%
  ungroup()
...
```{r}
plot_ecdf_of_time(all_results_common_to_padm_and_padm_interdiction)
```

#### Optimality gap 

```{r}
plot_ecdf_of_gap_wrt_best_known(all_results %>% filter(method == "PADM-interdiction"))
plot_ecdf_of_gap_wrt_best_known(solved_by_both)
```
