...
#  sub_data = data[data$norm == norm & data$warm_start == 0, ]
  
  # Create the bar chart of counts of status
#  plot = ggplot(sub_data, aes(x = reason, fill = reason)) + 
#    geom_bar() +  # Create a bar chart to count each status
#    labs(title = paste0("Count of Status for ", norm, " norm"),
#         x = "Status",
#         y = "Count",
#         fill = "Status") +
#    theme_minimal() +
#    theme(legend.position = "none") +  # Remove the legend as fill will match x-axis categories
#    facet_wrap(~ n_mutable_columns, ncol = 3)  # Facet by mutable_columns, 3 columns
