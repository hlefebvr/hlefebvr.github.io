...
write_ecdf_csv(all_results_wait_and_see, "total_time", "uflp-ecdf-time.csv")
write_ecdf_csv(all_results_wait_and_see, "gap", "uflp-ecdf-gap.csv")
...
summary_table <- all_results_wait_and_see %>%
  ...
  summarise(
    ...
    n_instances = n(),
    avg_total_time = median(total_time, na.rm = TRUE),
    avg_master_share = median(master_time / total_time * 100, na.rm = TRUE),
    avg_n_iterations = median(n_iterations, na.rm = TRUE),
    .groups = "drop"
  )
