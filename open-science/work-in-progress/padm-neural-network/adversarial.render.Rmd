...
data = data %>% mutate(time = ifelse(time >= 60, 60, time))
...
data = data %>% mutate(method = ifelse(method == "padm", "PADM", method))
data = data %>% mutate(method = ifelse(method == "padm-warm-start", "PADM", method))
data = data %>% mutate(method = ifelse(method == "gurobi-10s", "Gurobi (10s)", method))
data = data %>% mutate(method = ifelse(method == "gurobi-first-found", "Gurobi (First Found)", method))
