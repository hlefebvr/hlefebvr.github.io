...
```{r}
performance_profile = plot_performance_profile(results, criterion_column = "total_time", solver_column = "method") +
...
results_solved = results %>%
  ...
  group_by(method, n_servers, n_clients, p, deviation) %>%
  summarize(
    solved = n(),
...
```{r}
final_result <- merge(results_solved, results_unsolved, by = c("method", "n_servers", "n_clients", "p", "deviation"), all = TRUE)
final_result <- merge(final_result, results_fail, by = c("method", "n_servers", "n_clients", "p", "deviation"), all = TRUE)
```
...
for (method in unique(results$method)) {
  ...
  p = ggplot(results[results$method == method & results$unsolved == FALSE,], aes(x = iteration_count)) +
    geom_bar() +
    labs(
      title = method,
