...
results = results[!(results$n_facilities == 25 & results$n_customers == 25),]
```
...
```{r}
for (method in unique(subset$method)) {
  
  p = ggplot(subset[subset$method == method,], aes(x = iteration_count)) +
    geom_bar() +
    labs(
      title = method,
      x = "Number of iterations",
