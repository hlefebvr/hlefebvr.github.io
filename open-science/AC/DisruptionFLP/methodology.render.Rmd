...
for (ratio in unique(all_results$ratio)) {
  performance_profile(all_results[all_results$ratio == ratio,], main = paste("With mu =", ratio / 100))
}
```
...
plot_solved = function (dataset, main = "Solved instances") {
  ...
  barplot( data , beside = TRUE, col = colors$color, main = main, ylim = c(0, 30), cex.names=.8)
  legend("topleft",  legend = c("B&C", "CCG"), fill = colors$color)
}
```

```{r, dev = c("png", "pdf")}
#for (gamma in unique(Table$gamma)) {
#  plot_solved(Table[Table$gamma == gamma,], main = paste("Solved instances for Gamma =", gamma))
#}
```
