...
For example, `results_benders.csv` reads.

```{r echo = FALSE}
raw_benders = read.csv("./results_benders.csv", header = FALSE)
...
```{r}
benders = read_csv_benders("./results_benders.csv")
benders_IIS = read_csv_benders("./results_benders_IIS.csv")
```
...
read_csv_ccg = function (filename) {
  ...
  # Read raw results
  raw_results = read.csv("./results_ccg.csv", header = FALSE)
...
summary_benders_IIS = compute_summary_by_group(benders_IIS)
...
unsolved_benders_IIS = compute_unsolved_by_group(benders_IIS)
...
total_benders_IIS = compute_instances_by_group(benders_IIS)
...
```{r echo = FALSE}
digits = c(as.vector(matrix(0, nrow = length(group_by))), 0, 0, 0, 0, 0, 0, 2, 2, 2)
knitr::kable(Table1,
             digits = digits,
             col.names = c(str_group_by, "Benders", "Benders IIS", "CCG", "Benders", "Benders IIS", "CCG", "Benders", "Benders IIS", "CCG")
             ) %>%
      kable_classic() %>%
      add_header_above(c(" " = length(group_by), "Total" = 3, "Unsolved" = 3, "Time" = 3))
```
...
all_approaches = add_approach(all_approaches, benders_IIS, suffix = "iis")
all_approaches = add_approach(all_approaches, benders, suffix = "benders")
```
...
We introduce function `plot_performance_profile` which plots the performance profile over a restricted dataset.

```{r}
plot_performance_profile = function(data, xlim = c(1, 500), color = c("blue", "green", "red", "black"), main = "Performance profile") {
...
### Number of nodes between Benders and Benders IIS

#### Computations

```{r}
group_benders_by = c("gamma", "n_knapsacks", "n_items")
all_benders = merge(benders, benders_IIS, by = group_benders_by)
...
{
  xlim = c(1, 200)
  plot(profile.benders, col = "blue", xlim = xlim, main ="")
  lines(profile.iis, col = "green", xlim = xlim, main = "")
