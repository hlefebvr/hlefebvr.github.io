...
plot_gap = function(instance) {
  ...
  plot(dataset$Gamma, dataset$gap_OPT_ADR, type = "o", col = "red", main = instance)
...
plot_times = function(dataset, main = NULL, xlab = NULL, ylab = NULL) {
    ...
    for (method in methods) {
      ...
      plot_function(ecdf(result[,method]), col = current_color, lty = "solid", cex = 0, main = "", xlab = "", ylab = "", xlim = c(0, 100))
...
plot_instance = function(instance, field) {
  
  dataset = results[results$instance == instance,]
  opt = dataset[dataset$method == "OPT",]
  adr = dataset[dataset$method == "ADR",]
  
  plot(opt$Gamma, opt[[field]], main = "", xlab = "", ylab = "", type = "o", col = "blue")
  points(adr$Gamma, adr[[field]], type = "o", col = "red")
  
  title(main = instance,
        xlab = "Gamma",
        ylab = field)
  
  legend("topleft", legend = c("OPT", "ADR"), col = c("blue", "red"), pch = 16)
  
}

for (instance in unique(results$instance)) {
  plot_instance(instance, "time");
  plot_instance(instance, "best_obj");
}
